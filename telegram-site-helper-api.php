...
switch ($act) {
    ...
    case 'newChat':
		...
		$sth = $db->prepare("SELECT managerTelegramId, managerId FROM telegramSiteHelperManagers WHERE mainManager=:mainManager");
        $sth->execute([":mainManager" => 1]);
		$answer = $sth->fetch();
		$mainManagerId = $answer["managerTelegramId"];
		
        $sth = $db->prepare("INSERT INTO telegramSiteHelperChats (chatId, chatManager, chatCustomerName, chatCustomerPhone) VALUES (:chatId, :chatManager, :chatCustomerName, :chatCustomerPhone)");
        $sth->execute([
            ":chatId"            => $chatId,
			":chatManager"       => $mainManagerId,
        ...
        $msgText = json_encode(["text" => "Добрый день! Спрашивайте,что Вас интересует?"]);
        $sth->execute([
            ":msgChatId" => $chatId,
            ":msgFrom"   => $answer["managerId"],
            ":msgTime"   => time(),
            ":msgText"   => $msgText,
        ]);
    ...
    case 'editChat':
        ...
        $sth = $db->prepare("SELECT managerTelegramId FROM telegramSiteHelperManagers WHERE mainManager=:mainManager");
        $sth->execute([":mainManager" => 1]);
        while ($answer = $sth->fetch()) {
            $managerToSend[] = $answer["managerTelegramId"];
        }
    ...
    case 'sendMessage':
        ...
        if ($message != null) {
            ...
            $userInputEncoded = json_encode(array($chatId => $message));
            $output = exec('python webpage_message_handler.py ' . escapeshellarg($userInputEncoded));
            echo $output;
            /* echo $output; */
